...
foreach_in_collection mim [get_plan_groups $ICC_MIM_MASTER_LIST] {
  ...
  if { $mim_master_name == "bp_tile_node" } {
    ...
    set_fp_relative_location -name fe_tag_mem_array_rl -target_cell fe_tag_mem_array -target_corner tr -anchor_corner tl -anchor_object icache_data_mem_array
    ...
    set dcache_tag_mem_list [get_fp_cells -of_objects $mim -filter "is_hard_macro&&full_name=~*dcache*tag_mem*"] -legal orientations W
    ...
    for {set row 0} {$row<2} {incr row} {
      ...
      for {set column 0} {$column<8} {incr column} {
        set idx [expr $row*4+$column]
        append_to_collection macro_row [get_fp_cells -of_objects $mim -filter "is_hard_macro&&full_name=~*l2s*data_mem*$idx*"]
      }
      lappend l2s_data_mem_list $macro_row
    }

    set l2s_tag_mem_list [get_fp_cells -of_objects $mim -filter "is_hard_macro&&full_name=~*l2s*tag_mem*"] -legal orientations W
    set_fp_macro_options $l2s_tag_mem_list -legal_orientations W
    set_fp_macro_array -name l2s_tag_mem_array -elements $l2s_tag_mem_list -align_edge t -use_keepout_margin
    set_fp_relative_location -name l2s_tag_mem_array_rl -target_cell l2s_tag_mem_array -target_corner tr -anchor_corner tl -anchor_object l2s_data_mem_array
  }
}
