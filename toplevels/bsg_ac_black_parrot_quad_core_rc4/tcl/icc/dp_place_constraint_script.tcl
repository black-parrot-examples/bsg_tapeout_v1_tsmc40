...
foreach_in_collection mim [get_plan_groups] {
  ...
  if { $mim_master_name == "bp_tile_node" } {
    ...
    set fp_rf_list [sort_collection -dictionary [get_fp_cells -of_objects $mim -filter "is_hard_macro&&full_name=~*fp_regfile*"] name]
    set_fp_macro_options $fp_rf_list -legal_orientations N
    set_fp_macro_array -name fp_rf_array_$suffix -elements $fp_rf_list -use_keepout_margin -vertical
    set_fp_relative_location -name fp_rf_array_rl_$suffix -target_cell fp_rf_array_$suffix -target_corner bl -anchor_corner br -anchor_object int_rf_array_$suffix
