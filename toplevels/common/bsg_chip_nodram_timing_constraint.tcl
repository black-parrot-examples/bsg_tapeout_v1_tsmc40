...
# Usage example:
#
# source $::env(BSG_DESIGNS_DIR)/toplevels/common/bsg_chip_nodram_timing_constraint.tcl
#
...
proc bsg_chip_ucsd_bsg_332_nodram_timing_constraint {bsg_reset_port \
                                              bsg_core_clk_port \
                                              bsg_core_clk_name \
                                              bsg_core_clk_period \
  ...
  puts "INFO\[BSG\]: begin bsg_chip_ucsd_bsg_332_nodram_timing_constraint function in script [info script]\n"
  ...
  #set sdi_C_sclk_port [get_ports p_sdi_sclk_i[2]] ==> DRAM_PIN
  #set sdi_D_sclk_port [get_ports p_sdi_sclk_i[3]] ==> DRAM_PIN

  # output channel clock
  set sdo_A_sclk_port [get_ports p_sdo_sclk_o[0]]
  set sdo_B_sclk_port [get_ports p_sdo_sclk_o[1]]
  #set sdo_C_sclk_port [get_ports p_sdo_sclk_o[2]] ==> DRAM_PIN
  #set sdo_D_sclk_port [get_ports p_sdo_sclk_o[3]] ==> DRAM_PIN
 ...
 # create_clock -name sdi_C_sclk_BSG_CHIP_cdc \                                 ==> DRAM_PIN
 #              -period [get_attribute [get_clocks sdi_C_sclk] period] \
 #              -add $sdi_C_sclk_port

 # create_clock -name sdi_D_sclk_BSG_CHIP_cdc \                                 ==> DRAM_PIN
 ...
 #              -add $sdi_D_sclk_port
  ...
  #set_clock_uncertainty $in_io_clk_uncertainty [get_clocks sdi_C_sclk]   DRAM_PIN
  ...
  #set sdi_B_input_ports [add_to_collection [get_ports "p_sdi_B_data_i*"] [get_ports "p_sdi_ncmd_i*[1]"]] DRAM_PIN
  #set sdi_C_input_ports [add_to_collection [get_ports "p_sdi_C_data_i*"] [get_ports "p_sdi_ncmd_i*[2]"]]
  #set sdi_D_input_ports [add_to_collection [get_ports "p_sdi_D_data_i*"] [get_ports "p_sdi_ncmd_i*[3]"]] DRAM_PIN
  ...
  #set sdo_C_output_ports [add_to_collection [get_ports "p_sdo_C_data_o*"] [get_ports "p_sdo_ncmd_o[2]"]] DRAM_PIN
  #set sdo_D_output_ports [add_to_collection [get_ports "p_sdo_D_data_o*"] [get_ports "p_sdo_ncmd_o[3]"]] DRAM_PIN
  ...
  #set_input_delay -clock sdi_B_sclk -min $min_input_delay $sdi_B_input_ports     ==> DRAM_PIN
  #set_input_delay -clock sdi_C_sclk -min $min_input_delay $sdi_C_input_ports
  #set_input_delay -clock sdi_D_sclk -min $min_input_delay $sdi_D_input_ports     ==> DRAM_PIN
  ...
  #set_input_delay -add_delay \                                                   ==> DRAM_PIN
  #                -clock_fall \
  #                -clock sdi_B_sclk \
...
# ================> DRAM_PIN
  ...
  puts "INFO\[BSG\]: end bsg_chip_ucsd_bsg_332_nodram_timing_constraint function in script [info script]\n"

}

proc bsg_chip_nodram_timing_constraint {args} {
  ...
  if {$bsg_package != "ucsd_bsg_332" || $bsg_core_clk_period <= 0 || $bsg_master_io_clk_period <=0} {
    ...
    bsg_chip_ucsd_bsg_332_nodram_timing_constraint \
                     $bsg_reset_port \
                     $bsg_core_clk_port \
                     $bsg_core_clk_name \
                     $bsg_core_clk_period \
...
define_proc_attributes bsg_chip_nodram_timing_constraint \
